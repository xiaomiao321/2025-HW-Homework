...
{
  // UART6串口绘图
  UART_Send_Data(&huart6, UART6_Tx_Data, 1 + 12 * sizeof(float));
}
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
  ...
  {
    UART2_Manage_Object.Callback_Function(UART2_Manage_Object.Rx_Buffer,
      UART2_Manage_Object.Rx_Buffer_Size);
    HAL_UARTEx_ReceiveToIdle_DMA(huart, UART2_Manage_Object.Rx_Buffer,
      UART2_Manage_Object.Rx_Buffer_Size);
  }
  else if (huart->Instance == USART3)
  {
    UART3_Manage_Object.Callback_Function(UART3_Manage_Object.Rx_Buffer,
      UART3_Manage_Object.Rx_Buffer_Size);
    HAL_UARTEx_ReceiveToIdle_DMA(huart, UART3_Manage_Object.Rx_Buffer,
      UART3_Manage_Object.Rx_Buffer_Size);
  }
  else if (huart->Instance == UART4)
  {
    UART4_Manage_Object.Callback_Function(UART4_Manage_Object.Rx_Buffer,
      UART4_Manage_Object.Rx_Buffer_Size);
    HAL_UARTEx_ReceiveToIdle_DMA(huart, UART4_Manage_Object.Rx_Buffer,
      UART4_Manage_Object.Rx_Buffer_Size);
  }
  else if (huart->Instance == UART5)
  {
    UART5_Manage_Object.Callback_Function(UART5_Manage_Object.Rx_Buffer,
      UART5_Manage_Object.Rx_Buffer_Size);
